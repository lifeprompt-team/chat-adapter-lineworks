...
Receive LINE WORKS Bot callbacks and send LINE WORKS text messages from Chat SDK handlers.
...
- LINE WORKS `message` events with `content.type === "text"`.
- Direct user messages and channel messages.
- `postMessage()` for LINE WORKS text messages.
- `openDM()` and `isDM()`.
...
- Non-text events are logged and ignored.
