  ...
  implements Adapter<LineWorksThreadId, unknown>
{
  ...
  async postMessage(
    ...
    if (!buttonTemplate && text.length > MAX_TEXT_LENGTH) {
    ...
    if (buttonTemplate) {
      raw =
        destination.kind === "user"
          ? await this.client.sendUserContent(destination.userId, buttonTemplate)
          : await this.client.sendChannelContent(destination.channelId, buttonTemplate);
...
function encodePostbackData(args: { actionId: string; value?: string }): string {
  if (args.actionId.includes(ACTION_DATA_DELIMITER)) {
    throw new ValidationError("lineworks", "LINE WORKS action id must not include a newline");
  }
  const encoded =
...
function buildLineWorksButtonTemplate(
  ...
  if (contentText.length === 0) {
    throw new ValidationError("lineworks", "LINE WORKS button template content text is required");
  }
...
function extractCardElement(message: AdapterPostableMessage): CardElement | null {
...
function toLineWorksMessageAction(action: ButtonElement): LineWorksTemplateAction | null {
  if (action.disabled) {
    return null;
  }
...
function toLineWorksUriAction(action: LinkButtonElement): LineWorksTemplateAction {
  validateButtonLabel(action.label);
  return {
    label: action.label,
    type: "uri",
...
function validateButtonLabel(label: string): void {
  if (label.length === 0) {
    throw new ValidationError("lineworks", "LINE WORKS button label is required");
  }
...
function collectCardChildText(child: CardChild): string[] {
  switch (child.type) {
    ...
    case "section":
      return child.children.flatMap((sectionChild) => collectCardChildText(sectionChild));
    case "fields":
